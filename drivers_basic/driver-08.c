...
/* When a process sleeps, it does so in expectation that some
   ...
   If a wait_event is used, the process is put into an uninterruptible
   sleep which is usually not desired. The preferred alternative is
   wait_event_interruptible, which can be interrupted by signals.
   This version returns an integer value that should be checked; a
   nonzero value means the sleep was interrupted by some signal, and
   the driver should probably return -ERESTARTSYS.
   ...
   The other half of the picture is waking up. Some other thread of
   execution (a different process, or an interrupt handler, perhaps)
   has to perform the wakeup since the process to wake up is
