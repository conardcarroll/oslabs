...
int incrementer(void *ptr) {
 	...
 	while (idx < 1000000) {
			printk(KERN_INFO "Consumer %1d idx %d\n" , (int)ptr, idx);
		if(!down_interruptible(&lock)) {
			printk(KERN_INFO "Consumer %1d entered lock\n" , (int)ptr);
			if(idx >= 1000000) {
				printk(KERN_INFO "Consumer %1d broke out \n" , (int)ptr);
				break;
			}
			arr[idx++] += 1;
			schedule();
			...
			printk(KERN_INFO "Consumer %1d upped \n" , (int)ptr);
...
void cleanup_module(void) {
	...
	printk(KERN_EMERG "t1 no stop");
	

	if(stop2 == 0) {
		kthread_stop(t2);
		printk(KERN_EMERG "t2 stop");
	}
	printk(KERN_EMERG "t2 no stop");
