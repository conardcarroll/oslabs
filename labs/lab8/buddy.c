...
int buddy_init(int size)
{
    ...
    buddy_pool = kmalloc(size, GFP_KERNEL);
    if (!buddy_pool) {
        return -1;
    }

    /* allocate the head node of the tree */
    buddy_head = (node_t*)kmalloc(sizeof(node_t), GFP_KERNEL);
    if (!buddy_head) {
        return -1;
    }
...
int _buddy_alloc(node_t *n, int size)
{
    ...
    if (n->size / 2 >= size) {
        ...
        n->left = (node_t*)kmalloc(sizeof(node_t), GFP_KERNEL);
        n->right = (node_t*)kmalloc(sizeof(node_t), GFP_KERNEL);
        n->left->state = FREE;
        n->right->state = FREE;
